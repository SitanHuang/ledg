...
async function cmd_budget(args) {
  ...
  cmd_report_modifiers.from = entry_datestr(from);
  cmd_report_modifiers.to = entry_datestr(to);
...
//     let d = Math.max(b - a, b - x);
    ...
    let cRate = Math.round(x / prdsPsd * 100) / 100;
    let bRate = Math.round(d / remainPrds * 100) / 100;
  ...
  let balanceData = await report_sum_accounts(args, true, budget.budgets);
  let forkedBudgets = {};
  ...
  console.log(tabulate(table, {
    align: [TAB_ALIGN_LEFT, TAB_ALIGN_RIGHT, TAB_ALIGN_LEFT, TAB_ALIGN_LEFT, TAB_ALIGN_LEFT, TAB_ALIGN_RIGHT, TAB_ALIGN_RIGHT, TAB_ALIGN_RIGHT, TAB_ALIGN_RIGHT, TAB_ALIGN_RIGHT, TAB_ALIGN_RIGHT],
    colBorder: ' ',
