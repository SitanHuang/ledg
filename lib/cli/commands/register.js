...
function _cmd_register_group(args, data, skipTo, depth, int, q, sortFunc) {
  ...
  while (crntD < to) {
    ...
    endDate.setFullYear(crntD.getFullYear() + int[0]);
    endDate.setMonth(crntD.getMonth() + int[1]);
    endDate.setDate(crntD.getDate() + int[2]);

    let accs = {};
    for (let i = 0;i < data.length;i++) {
      let e = data[i];
      if (e.time >= Math.floor(endDate / 1000) || e.time < Math.floor(crntD / 1000))
        continue;
      for (let q of args.accounts) {
        for (let t of e.transfers) {
          if (t[1].match(q)) {
            let a = print_truncate_account(t[1], depth);
            let valDate = args.flags['valuation-eop'] ?
                            (endDate / 1000 - 1) : e.time;
            accs[a] = (accs[a] || new Money()).plus(t[2].tryConvertArgs(args,valDate));
          }
        }
      }
    }
    let keysSorted = Object.keys(accs).sort();

    let j = -1;
    for (let acc of keysSorted) {
      j++;
      let amnt = accs[acc];
      let row = j == 0 || args.flags.format || args.flags.sort ?
            [ c.cyanBright(entry_datestr(endDate / 1000)) ] : [''];

      row.push(isNaN(mdep) ? acc : report_transform_mindepth(acc, mdep));
    ...
    for (let q of args.accounts) {
      for (let t of e.transfers) {
        if (t[1].match(q)) {
          ...
          let acc = print_truncate_account(t[1], depth);
          row.push(desc, isNaN(mdep) ? acc : report_transform_mindepth(acc, mdep));
