async function cmd_add(args, modifyMode=false) {
  ...
  if (opts.pending !== false && !modifyMode && args.flags['default-pending'])
    opts.pending = true;
  ...
  for (let i = 0;i < _.length;i++) {
    ...
    if (v.match(/^((([+-]?\d+(\.\d+)?)\s*([^\d\s,*\-.]+)|([^\d\s,*\-.]+)\s*([+-]?\d+(\.\d+)?)|([+-]?\d+(\.\d+)?))(, ?)?)+$/))
      try {
        num = Money.parseMoney(v, opts.time || Math.floor(new Date() / 1000));
      } catch (e) {}

    if (v == '*') {
      delete opts.pending;
    } else if (opts.pending !== false && v == '!') {
      opts.pending = true;
  ...
  if (entry.transfers)
    console.log("\n" + print_entry_ascii(entry, undefined, {
      flags: {
        dp: args.flags.dp
      }
    }));
  ...
  // empty
  if (transfers.length == 0 && !modifyMode && !opts.event) {
