...
async function cmd_history(args) {
  ...
  while (crntD < to) {
    ...
    let nextDate = new Date(crntD);
    nextDate.setFullYear(crntD.getFullYear() + int[0]);
    nextDate.setMonth(crntD.getMonth() + int[1]);
    nextDate.setDate(crntD.getDate() + int[2]);

    let row = [];

    if (args.flags.epoch) {
      row.push(nextDate.getTime().toString());
    } else if (args.flags.isofull) {
      row.push(nextDate.toISOString().split('T')[0]);
    ...
    for (let acc of accounts) {
      ...
      row.push(new TabularHTMLNumberCell(v.colorFormat(dp)));
    }
  ...
  if (!args.flags.csv) {
    ...
    if (args.flags.avg) {
      ...
      for (let i = 0;i < rowL;i++) {
        ...
        if (j < -1) {
          ...
          let acc = accounts[j];
          let v = acc.sum.divPrim(dateFunctions.length || 1);
          ...
          avg.push(new TabularHTMLNumberCell(v.colorFormat(isNaN(dp) ? 2 : dp)));
        }
      }
      table.push(avg);
    }
      ...
      for (let i = 0;i < rowL;i++) {
        ...
        if (j < -1)
          ...
          if (asum.indexOf(j) >= 0) {
            ...
            sum.push(new TabularHTMLNumberCell(sums[j].colorFormat(isNaN(dp) ? 2 : dp)));
          } else
            sum.push('');
        }
      }
    ...
    let title = [[`Reporting from ${c.bold(strF)} to ${c.bold(strT)}\n`]];
    title[0].header = false;
    title[0][0].htmlTitle = 'h3';
