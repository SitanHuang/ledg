async function cmd_accounts(args) {
  ...
  if (tree) {
    ...
    accTree.list.forEach((x) => {
      ...
      sum = sum.plus(balanceData[accTree.fullList[i]]);
  ...
  sum = sum.toNumber();
  if (args.flags.sum) console.log(c.bold(`${print_pad_right('Sum', width)} ${print_pad_left(print_color_money(sum), width2, print_format_money(sum).length)}`));
  console.log("");
}
