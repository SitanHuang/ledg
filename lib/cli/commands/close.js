async function cmd_close(args) {
  ...
  let date = new Date(q.queries[0].from * 1000);
  let desc = entry_datestr(date / 1000).replace(/-01/g, '') + ' Book Close';

  date = new Date(q.queries[0].to * 1000);
  date = entry_datestr(date / 1000);
