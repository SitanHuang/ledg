...
async function cmd_burndown(args) {
  if (!DEBUG && (process.stdout.columns <= 10 || process.stdout.rows <= 15)) {
  ...
  let argQueries = Object.keys(args.flags).sort().filter(x => x.match(/q\d+/)).map(x => {
    ...
    args2.flags['valuation-date'] = args2.flags['valuation-date'] || args.flags['valuation-date'];
    args2.flags['valuation-eop'] = eop;
    return args2;
  }).filter(x => !!x);
  ...
  let query = {
    cumulative: args.flags.cumulative && argQueries.length,
  ...
  if (!DEBUG && intervals > maxIntervals) {
    ...
    for (let j = 0;j < argQueries.length;j++) {
      ...
      if (!args.flags.count)
        row[j] = row[j].val(args.flags.currency || data.defaultCurrency,
                            eop ? (query.queries[i].to - 1) : undefined);
  ...
  let chart = new BarChart(min, max, _d);
