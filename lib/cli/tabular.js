...
function tabulate(data, opts) {
  ...
  if (dcols != null) { // covers undefined too
    let dropped = 0;
    dcols.toString().split(",").map(x => parseInt(x) - 1).sort().forEach(x => {
      x -= dropped++;
  ...
  if (cli_args.flags.csv || (opts && opts.csv)) {
    ...
    return data.map(row => Array(width).fill('').map((x, i) => {
      let col = row[i] || '';
      let q = cli_args.flags['csv-no-quotes'] ? '' : '"';
      return q + strip_ansi((typeof col == 'object' && col.pop) ?
                                 col[0].toString() :
                                 col.toString())
                     .replace(/\"/g, "\"\"") + q;
    }).join(cli_args.flags['csv-delimiter'] || ",")).join("\n");
  }
  ...
  for (let i = 0;i < data.length;i++) {
    ...
    if (opts.alternateColor)
      if (opts.alternateColor == 2)
        rowData = print_alternate_row(rowData, row.rowIndex);
      else
