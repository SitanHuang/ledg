...
async function index() {
  ...
  if (fs_book_name == '-') {
    ...
    if (fs.existsSync(fs_book_name + '.config.ledg')) {
      // also loads price & include files
  ...
  let include_books = cli_args.flags['include-books'];
  if (include_books) {
    include_books = include_books.toString().split(",");
    for (let x of include_books)
      await fs_read_include(x.trim());
  }

  // update fs_data_range
  fs_append_data_range();
