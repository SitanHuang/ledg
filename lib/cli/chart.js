class Chart {
  constructor(min, max, data) {
    ...
    this.height = process.stdout.rows - 5;
    if (cli_args.flags.weekly || cli_args.flags.biweekly)
      this.height -= 2;
  ...
  render() {
    return '\n' + this.buffer
      .map((r, i) => i == this.zeroRow ? c.underline(r.join("")) : r.join(""))
      .join("\n");
  }
}
