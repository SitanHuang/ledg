...
function print_entry_ascii(entry, maxWidth, args=cli_args) {
  ...
  maxWidth = maxWidth || print_max_width_from_entries([entry], args);
  let alignrightwidth = 2 + maxWidth.transDesc + (maxWidth.transDesc ? 2 : 0) + maxWidth.acc + 2 + maxWidth.mon - 9 - 10 - (entry.description ? entry.description.length : 0) - (entry.pending ? 2 : 0) - (entry.event ? 7 + entry.event.toString().length : 0);
  let spacePad = Array(Math.max(alignrightwidth, maxWidth.desc + 2 - (entry.description ? entry.description.length : 0) - (entry.pending ? 2 : 0) - (entry.event ? 7 + entry.event.toString().length : 0))).fill(' ').join('');
  let str = c.cyanBright.bold(entry.time ? entry_datestr(entry) : '[time]') +
            (entry.pending ? c.redBright.bold(' ! ') : ' ') +
            (entry.event ? c.cyanBright.bold('event ') + c.yellowBright(entry.event) + ' ' : '') +
...
function print_max_width_from_entries(entries, args=cli_args) {
  ...
  while (len--) {
    ...
    if (!cli_args.flags['tc-expose'] && e.clockIn && !e.clockOut && e.transfers.length == 0) {
      ...
      a.desc = Math.max(a.desc, el + (e.pending ? 2 : 0) + (e.event ? 7 + e.event.toString().length : 0));
      ...
      while (len2--) {
        ...
        a.mon = Math.max(a.mon, e.transfers[len2][2]
                                 .tryConvertArgs(cli_args, e.time)
                                 .noColorFormat().length);
      }
    }
  }
  return a;
...
function print_progress_bar(perc, opts) {
  if (isNaN(perc))
    return Array(opts?.width || 50).fill(' ').join('');
...
const print_weekdayNames = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday',
  'Friday', 'Saturday' ];
