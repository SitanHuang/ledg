...
function print_entry_ascii(entry, maxWidth, args=cli_args) {
  ...
  let alignrightwidth = 2 + maxWidth.transDesc + (maxWidth.transDesc ? 2 : 0) + maxWidth.acc + 2 + maxWidth.mon - 9 - 10 - (entry.description ? entry.description.length : 0) - (entry.pending ? 3 : 0);
...
function print_max_width_from_entries(entries) {
  ...
  while (len--) {
    ...
    a.desc = Math.max(a.desc, el + (e.pending ? 3 : 0));
