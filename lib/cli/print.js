...
function print_entry_ascii(entry, maxWidth, args=cli_args) {
  ...
  let str = c.cyanBright.bold(entry.time ? entry_datestr(entry) : '[time]') + (entry.pending ? c.redBright.bold(' ! ') : ' ') + (typeof entry.description == 'string' ? entry.description : '[title]').trim() +
            Array(Math.max(alignrightwidth, maxWidth.desc + 2 - (entry.description ? entry.description.length : 0) - (entry.pending ? 2 : 0))).fill(' ').join('') + c.cyan(entry.uuid ? (' #' + (entry.uuid)) : ' [uuid]');
...
function print_entry_title(e) {
  /*if (!cli_args.flags['tc-expose'] && e.clockIn && !e.clockOut && e.transfers.length == 1) {
    return 'i ' + c.cyanBright(entry_timestr(e.clockIn)) + ' ' +
           c.yellowBright.bold(e.description.trim()) +
           c.cyan(' #' + e.uuid);
  } else if (!cli_args.flags['tc-expose'] && e.clockOut && !e.clockIn && e.transfers.length == 2) {
    return (e.pending ? 'o' : 'O') + ' ' + c.cyanBright(entry_timestr(e.clockIn)) + ' ' +
           c.cyan(' #' + e.uuid);
  }*/
...
function print_max_width_from_entries(entries) {
  ...
  while (len--) {
    ...
    if (!cli_args.flags['tc-expose'] && e.clockIn && !e.clockOut && e.transfers.length == 1) {
      a.desc = Math.max(a.desc, el + 9 + 2);
    } else if (!cli_args.flags['tc-expose'] && e.clockOut && !e.clockIn && e.transfers.length == 2) {
      a.desc = Math.max(a.desc, 9 + 2);
    } else {
