...
function argsparser(_args) {
  ...
  for (let i = 0;i < _args.length;i++) {
    ...
    if (!match.length && (match = arg.match(/^[a-z0-9]{8}$/i))) {
      ...
      if (val !== '' && val !== true && !isNaN(Number(val))) val = Number(val);
      if (val == 'true') val = true;
      if (val == 'false') val = false;
      args.flags[key] = val;

      if (ARG_FORMATS_OPTIONS.indexOf(args.flags.format) < 0)
        delete args.flags.format;

      if (ARG_FORMATS_OPTIONS.indexOf(key) >= 0)
        args.flags.format = key;

      if (args.flags.format)
        args.flags[args.flags.format] = true;
  ...
  // smart dates period
  let period = args.flags.period || args.flags.P;
  period && report_proc_period_opts(args, period);

  if (uuids.length)
    args.modifiers['uuid'] = args.modifiers.uuid || uuids.join("|");
