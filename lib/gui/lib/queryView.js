class QueryView extends TabView {
  ...
  async updateQueryContent(hideAdvanced) {
    ...
    <form post="#" onsubmit="return false">
          ...
          <x-box>
            ...
            <x-select class="interval">
              <x-menu>
                ...
                <x-menuitem value="--weekly" toggled>
                  <x-label>Weekly</x-label>
                </x-menuitem>

                <x-menuitem value="--daily">
      ...
      <x-accordion ${hideAdvanced ? 'style="display:none"' : ''} ${this._accordionOpened ? 'expanded' : ''}>
        <header>
