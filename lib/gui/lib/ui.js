...
function ui_update_menu() {
  ...
  $mbud.find('> x-menu > x-menuitem').each((i, val) => {
    ...
    budgets.forEach(x => {
      html += `<x-menuitem onclick='ui_open_tabview(${type}, { budget: ${escHtml(JSON.stringify(x))} })'><x-label>${escHtml(x)}</x-label></x-menuitem>`;
    });
...
class TabView {
  ...
  onClose() {
    ...
    let i = tabViews.indexOf(this);
    i >= 0 && tabViews.splice(i, 1);
  }
}