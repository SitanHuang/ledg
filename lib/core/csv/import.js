async function import_csv_exec(code, path) {
  ...
  ctx.amount = (x) => Money.parseMoney(x).serialize(true);
  ctx.invert = (x) => Money.parseMoney(x).timesPrim(-1).serialize(true);

  ctx.account = (x, map) => {
    if (!map)
      return x;
    for (let q in map)
      if (x.match(new RegExp(q)))
        return map[q];
    return ctx.default_account;
  };
