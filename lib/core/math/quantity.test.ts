...
describe('Quantity', () => {
  ...
  describe('Arithmetic operations', () => {
    describe('plus()', () => {
      ...
      it('should add a number to a quantity correctly', () => {
        ...
        expect(result.eq(new Quantity(1n, 1n))).toBe(true);
      });

      it('should handle addition that cancels out to zero', () => {
        // 5/7 + (-5/7) should equal 0
        const a = new Quantity(5n, 7n);
        const b = new Quantity(-5n, 7n);
        const result = a.plus(b);
        expect(result.eq(new Quantity(0n, 1n))).toBe(true);
      });

      it('should be commutative', () => {
        // a + b should equal b + a
        const a = new Quantity(2n, 5n);
        const b = new Quantity(3n, 7n);
      ...
      it('should correctly add quantities created from floating-point numbers', () => {
        ...
        const a = Quantity.fromNumber(0.1);
        const b = Quantity.fromNumber(0.2);
        const result = a.plus(b);
    ...
    describe('minus()', () => {
      ...
      it('should subtract a number from a quantity correctly', () => {
        ...
        expect(result.eq(new Quantity(2n, 4n))).toBe(true);
      });

      it('should handle subtraction resulting in a negative value', () => {
        // 1/3 - 2/3 = -1/3
        const a = new Quantity(1n, 3n);
        const b = new Quantity(2n, 3n);
        const result = a.minus(b);
        expect(result.eq(new Quantity(-1n, 3n))).toBe(true);
      });
    ...
    describe('times()', () => {
      ...
      it('should multiply a quantity by a number correctly', () => {
        ...
        expect(result.eq(new Quantity(6n, 4n))).toBe(true);
      });
      ...
      it('should handle multiplication with negative quantities', () => {
        ...
        const b = new Quantity(2n, 3n);
        const result = a.times(b);
        expect(result.eq(new Quantity(-6n, 12n))).toBe(true);
      });

      it('should multiply two negative quantities to yield a positive result', () => {
        // (-1/2) * (-3/4) = 3/8
        const a = new Quantity(-1n, 2n);
        const b = new Quantity(-3n, 4n);
        const result = a.times(b);
        expect(result.eq(new Quantity(3n, 8n))).toBe(true);
      });

      it('should handle multiplication by zero', () => {
        const a = new Quantity(3n, 5n);
    ...
    describe('div()', () => {
      ...
      it('should throw an error when division by a zero Quantity is attempted', () => {
        const a = new Quantity(1n, 2n);
        expect(() => a.div(new Quantity(0n, 1n))).toThrow("Division by zero");
      });
    ...
    test('plus: Adding mixed fractions (different denominators)', () => {
      ...
      // eq check:
      expect(sum.eq(new Quantity(7n, 6n))).toBe(true);
    ...
    test('plus: Adding negative number (sign carried in numerator)', () => {
      ...
      const negative = new Quantity(-5n, 1n);
      const positiveFraction = Quantity.parse('3.5').reduce(); // => 35/10 => 7/2
    ...
    test('minus: Subtracting two integer-based Quantities', () => {
      const q1 = new Quantity(10n, 1n);
      const q2 = new Quantity(6n, 1n);
    ...
    test('minus: Subtracting fractions, expecting negative result', () => {
      ...
      const quarter = new Quantity(1n, 4n);
      const threeQuarters = new Quantity(3n, 4n);
    ...
    test('times: Multiplying two positive integer Quantities', () => {
      const q1 = new Quantity(3n, 1n);
      const q2 = new Quantity(4n, 1n);
    ...
    test('times: Multiplying fractions and checking sign', () => {
      // (-3/4) * (1/2) => -3/8
      const negativeFraction = new Quantity(-3n, 4n);
    ...
    test('times: Multiplying large bigints from parsed strings', () => {
      const q1 = Quantity.parse('123456789123456789.3333');  // huge integer
      const q2 = Quantity.parse('987654321');           // large integer
    ...
    test('div: Large bigints from parsed strings', () => {
      const q1 = Quantity.parse('99999999999999999999');
      const q2 = Quantity.parse('3');
    ...
    test('Mixed usage: times + plus in a chain', () => {
      ...
      const q1 = new Quantity(3n, 2n); // 1.5
      const q2 = new Quantity(4n, 3n); // ~1.3333
      const chain = q1.times(q2).plus(5).reduce();

      expect(chain.numerator).toBe(7n);
      expect(chain.denominator).toBe(1n);
      expect(chain.eq(new Quantity(7n, 1n))).toBe(true);
    });
    ...
    test('Mixed usage: plus + minus + times + div in a single expression', () => {
      ...
      const onePointTwo = Quantity.fromNumber(1.2);  // => 12/10
      const three = Quantity.fromNumber(3);          // => 3/1
      const ninePointSix = Quantity.fromNumber(9.6); // => 96/10
      const four = Quantity.fromNumber(4);           // => 4/1
      const oneSixth = new Quantity(1n, 6n);   // => 1/6
    ...
    test('Denominator sign correction: ensuring sign is only carried by numerator', () => {
      ...
      const tricky = new Quantity(5n, -3n);
    ...
    test('Ensure cross-multiplication eq check works as intended (denominator differences)', () => {
      ...
      const qA = new Quantity(3n, 4n);
      const qB = new Quantity(6n, 8n);

      expect(qA.eq(qB)).toBe(true);
      expect(qA.valueOf()).toBe('0.75');

      // Also check that a difference is recognized:
      const qC = new Quantity(75n, 100n); // 0.75
      expect(qB.eq(qC)).toBe(true);

      const qD = new Quantity(76n, 100n); // 0.76
  ...
  test('toString removes trailing zeros in fractional part and always leaves at least one digit', () => {
    ...
    const qInteger = new Quantity(5n, 1n);
    ...
    const qNegInteger = new Quantity(-123n, 1n);
  ...
  describe('Rounding Tests (should mimic JavaScript Math.round behavior)', () => {
    test('round: rounds positive number exactly at half upward', () => {
      const q = Quantity.fromNumber(1.45);
    ...
    test('round: rounds negative number with non-half fractional part correctly', () => {
      const q = Quantity.fromNumber(-1.46);
      expect(q.round(1).toNumber()).toBe(-1.5);
      expect(q.round(0).toNumber()).toBe(-1);
