...
async function query_exec(query) {
  ...
  await data_iterate_books(data_books_required(_range_min * 1000, _range_max * 1000),
    ...
    // many commands such as edit and print
    // must traverse through in original order
    ENTRY: while (++len < book.length) {
      let e = book[len];
      if (e.time < query.from || e.time >= query.to)
        continue;
      if ((e.virt && cli_args.flags.real) ||
          (e.pending && cli_args.flags.cleared))
        continue;
      let i = -1;
      QUERY: for (let q of query.queries) {
        ...
        if (isSum)
          ...
          for (let t of e.transfers)
            for (let qt of q.accounts)
              if (t[1].match(qt)) {
                sum = sum.plus(q.invert ? t[2].timesPrim(-1) : t[2]);
                continue TRANSFER;
              }
...
async function query_args_exec(args, collect=[]) {
  return await query_exec({ queries: [ query_args_to_filter(args, collect) ] });
}
...
function query_args_to_filter(args, collect=[]) {
  ...
  x.from = Math.floor(Date.parse(report_replaceDateStr(args.modifiers.from || '@min') + 'T00:00:00') / 1000);
  x.to = Math.floor(Date.parse(report_replaceDateStr(args.modifiers.to || '@max') + 'T00:00:00') / 1000);
  report_compile_test_func(x);
  return x;
}
