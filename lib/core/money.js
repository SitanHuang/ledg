...
class Money {
  ...
  valBig(tCur=this.initCur, date=this.date) {
    return this.convert(tCur, date).amnts[tCur];
  }

  tryConvertArgs(args, date=this.date) {
    const cur = args.flags.currency;
    if (!cur)
      return this.clone();
    let d;
  ...
  colorFormat(dp=Big.DP, plus, pos=true) {
    let keys = Object.keys(this.amnts).sort();
    let str = [];
    for (let x of keys) {
  ...
  // returns ratio of this / b as Big
  div(b) {
    let iz1 = this.isZero();
    let iz2 = b.isZero();
    if (iz2 || iz1)
    ...
    return this.valBig(cur).div(b.valBig(cur));
  }
   ...
   * +num: if (this > b)
   *
   * exact=true if unresolved conversion returns null
   */
  compare(b, exact) {
    try {
      let iz1 = this.isZero();
      ...
      return exact ? null : 0; // uncomparable due to unresolved conversion
    }
  }
  ...
  isZero(precision=0) {
    ...
    try {
      return Math.abs(this.val(cur)) <= precision;
    } catch (e) {
      return false;
    }
  }
