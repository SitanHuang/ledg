...
function data_init_data() {
  return data = {
    ...
    booksOpened: {},
    budgets: {},
    precisionTolerance: 0.00001, // 0.005 in beancount
    defaultCurrency: '$',
    accountCurrency: {},
    priceFiles: [], // auto loaded by fs.js
    include: [], // auto loaded by fs.js, auto set doNotWrite:true
...
async function data_open_books(books) {
  for (let y of books) {
    if (data.booksOpened[y] >= DATA_BOOK_OPENED)
      continue;
    await fs_read_book(y);
    // fs_read_book could set it to dirty
    data.booksOpened[y] = Math.max(DATA_BOOK_OPENED, data.booksOpened[y]);
  }
}
...
async function data_iterate_books(books, callback, afterOpenCallback) {
  ...
  if (afterOpenCallback)
    await afterOpenCallback();

  for (let b of books) {
    let book = data.books[b];
    if (await callback(book) == DATA_CALLBACK_STOP)
      break;
  }
}
