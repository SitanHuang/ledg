class MultiPeriodAccReport {
  ...
  async query() {
    ...
    let oldCur = this.args.flags.currency;
    if (this.eop) {
    ...
    this.args.flags.currency = oldCur;
...
class MultiPeriodAccSubReport {
  ...
  makeTable(parent) {
    ...
    let mdep = parent.args.flags['min-depth']
    for (let i = 0;i < this.sumsTable.length;i++) {
      ...
      if (!isNaN(mdep)) {
        let name = report_transform_mindepth(srow.tree.fullName, mdep);
        row[0] = name.length ? tree ?
                   Array((srow.tree.depth - mdep + 1) * 2 - 1).fill(' ').join("")
                     + srow.tree.name :
                   name : '';
      } else {
        row[0] = tree ?
  ...
  calcAverage(parent) {
    ...
    for (let row of tab) {
      ...
      row.push(total.divPrim(row.length));
    }
    ...
    let len = parent.periods.length + (parent.args.flags.avg ? 1 : 0);
    row[len - 1] = total.divPrim(row.length);
  }

  resortWihoutTree(parent) {
    this.sumsTable = this.sumsTable.sort((a, b) =>
      (parent.args.flags.sort && b.tree.totalSum.compare(a.tree.totalSum)) ||
        a.tree.fullName.localeCompare(b.tree.fullName)
    );
  }
  ...
  putTreesToSumsTable(parent, trees) {
    ...
    for (let i = 0;i < trees.length;i++) {
      let tree = trees[i];
      let row = tree.dsum.map((x, i) => parent.eop ?
