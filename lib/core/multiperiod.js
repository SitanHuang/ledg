class MultiPeriodAccReport {
  ...
  processArgs() {
    ...
    args.flags.dp = isNaN(args.flags.dp) ? 2 : args.flags.dp;
  ...
  populateTree() {
    ...
    this.tree.sumForDepth(this.depth);
  ...
  makeTable() {
    ...
    */
    ...
    let that = this;
    net = ['  Net', ...(net.map(x => that.formatMoney(x)))];
    table.push(net);
  ...
  formatMoney(x) {
    let dp = Math.max(this.args.flags.dp, 0) || undefined;
...
class MultiPeriodAccSubReport {
  ...
  makeTable(parent) {
    ...
    for (let i = 0;i < this.sumsTable.length;i++) {
      ...
      row.header = i == this.sumsTable.length - 1;
      rows.push(row);
    }


    let sumRow = [''].concat(this.sums.map(x => this.formatMoney(parent, x)));
  ...
  formatMoney(parent, x) {
    let dp = Math.max(parent.args.flags.dp, 0) || undefined;
    let plus = this.plus;
