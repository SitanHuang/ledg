...
function report_replaceDateStr(dateStr) {
  ...
  let date = chrono.parseDate(dateStr);

  if (DEBUG)
    console.debug(`smart date: ${dateStr} => ${date}`);

  if (!date && dateStr && dateStr.length)
    throw `${dateStr} is not in ISO format nor parsable by chrono's smart date parser`;
  else if (!date)
    return dateStr;
...
function report_proc_period_opts(args, period) {
  ...
  let parsed = chrono.parse(period);

  if ((parsed = parsed[0]) && parsed.start)
    args.modifiers.from = entry_datestr(parsed.start.date().getTime() / 1000) ||
                          args.modifiers.from;
  else
    throw `smart date: --period requires a start date, and ${period} cannot be parsed`;

  if (parsed.end)
    args.modifiers.to = entry_datestr(parsed.end.date().getTime() / 1000) ||
                        args.modifiers.to;
}
