...
// this flag stops emitting error of uuid reassignment
var _fs_flag_reading_include = false;

async function fs_read_book(year) {
  let path = fs_book_name + '.' + year + '.ledg';
  // could be opened before by include-books
  if (!data.books[year])
    data.books[year] = [];

  await _fs_read_book(path, year);
}

async function fs_read_include(path) {
  _fs_flag_reading_include = true;
  await _fs_read_book(path, false, true);
  _fs_flag_reading_include = false;
}

function _fs_read_book(path, useYear, doNotWrite) {
  return new Promise((resolve, reject) => {
    ...
    if (fs_book_name != '-' && fs.existsSync(path)) {
      ...
      const commitEntry = (entry) => {
        ...
        if (doNotWrite)
          entry.doNotWrite = doNotWrite;
        // console.log("commit====");
        // console.log(entry.transfers.map(x => [x[0], x[1], x[2].toString()]));

        let year = useYear || new Date(entry.time * 1000).getFullYear();
        entry._year = year;

        if (data.books[year])
          data.books[year].push(entry);
        else
          data.books[year] = [entry];
        _fs_entries_read++;
      };
      ...
      rl.on('line', (line) => {
        try {
          entry = fs_read_book_proc_line(entry, line, commitEntry, stat, doNotWrite);
        } catch (e) {
          reject(e);
          rl.close();
        }
      ...
      rl.on('close', () => {
        ...
        if (DEBUG)
          console.debug(`Opened ${useYear || path} book in ${new Date() - _start}ms, ${_fs_entries_read} entries read so far`);
        resolve();
      });
    } else {
      if (DEBUG)
        console.debug(`Attempted to open ${path} but no physical file was read.`);
      resolve();
    }
  });
}

function fs_read_entries_from_string(str, doNotWrite) {
...
const _fs_event_regex = /^event\s+([^\s]+)\s+([^#]+)/;
// the doNotWrite flag disables flagging a book to set dirty (files in data.include config
// can be read without the *.YEAR.ledg books being opened, causing empty files to be written
function fs_read_book_proc_line(entry, line, commitEntry, stat, doNotWrite) {
  ...
  if ((line[0] == 'O' || line[0] == 'o') && (match = line.match(_fs_tc_checkout_regex))) {
    ...
    if (uuid) {
      ...
      if (!_fs_flag_reading_include && !err_ignores_list["timeclock-uuid-reassigned-warning"])
    ...
    //console.log(line.substring(0,10),entry.time,new Date(entry.time*1000),entry_timestr(entry));
    ...
    let _em;
    if (entry.description.startsWith('event') &&
       (_em = entry.description.match(_fs_event_regex))) {
      entry.event = _em[1];
      entry.description = _em[2].trim();
    }
    ...
    if (UUIDreassigned) {
      ...
      if (!doNotWrite)
        data.booksOpened[year] = DATA_BOOK_DIRTY;
      if (!_fs_flag_reading_include && !!err_ignores_list["uuid-reassigned-warning"])
