...
function fs_serialize_entry(entry, explicit) {
  ...
  for (let key in entry) {
    if (key == 'description' || key == 'time' ||
        key == 'uuid' || key == 'transfers') continue;
...
function fs_serialize_entry_ledger(entry, pad) {
  let str = entry_datestr(entry) + ' ' + entry.description.trim() +
              ' #' + entry.uuid;
