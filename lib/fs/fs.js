...
// adds years in data.books to fs_data_range if not already exist
function fs_append_data_range() {
    fs_data_range = Object.keys(data.books).map(x => Number(x))
                      .concat(fs_data_range)
    ...
    if (data.booksOpened[y] == DATA_BOOK_DIRTY) {
      ...
      await new Promise((resolve, reject) => {
        ...
        for (let i = 0;i < data.books[y].length;i++) {
          let e = data.books[y][i];
          if (!e.doNotWrite)
            file.write(fs_serialize_entry(e) + "\n");
        }
...
async function fs_attempt_load_config() {
  ...
  if (fs.existsSync(path)) {
    ...
    // read include files
    for (let path of data.include)
      await fs_read_include(dir + path);
  }
}
