...
async function fs_read_price(path) {
  ...
  if (fs.existsSync(path)) {
    ...
    const commitPrice = (price) => {
      ...
      tree.put(price.time, price.price ? ONE.div(price.price) : 0);
...
const _fs_price_directive_regex = /^P\s+(\d{4}-\d{2}-\d{2})\s+([^\d\s,\-.]+)\s+((-?[\d.]+)\s*([^\d\s,\-.]+)|([^\d\s,\-.]+)\s*(-?[\d.]+))\s*$/;
function fs_read_price_proc_line(price, line, commitPrice) {
  ...
  if (line[0] == 'P') { // start price
    ...
    let match = line.match(_fs_price_directive_regex);
    ...
    price = {
      time: Math.floor(Date.parse(date + 'T00:00:00') / 1000),
      c1: cur1,
      c2: cur2,
      price: p
    };
